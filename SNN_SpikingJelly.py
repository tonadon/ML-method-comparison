...
ann_model = SimpleCNN().to(device)
criterion = nn.BCELoss()
optimizer = optim.Adam(ann_model.parameters(), lr=1e-4)


ANN_CPU_PATH = "./ann_base_cpu.pt"
ANN_CUDA_PATH = "./ann_base_cuda.pt"
# Training loop 
...
if (os.path.exists(ANN_CPU_PATH)):
	...
	for epoch in range(num_epochs):
		...
		for inputs, labels in train_loader:
			...
			# Convert labels to float tensor and reshape to (batch, 1)
...
with torch.no_grad():
    for inputs, labels in test_loader:
        ...
        for t in range(T):
            out_t = snn_model(inputs)  # shape: (batch, 1)
            batch_outputs.append(out_t)
        outputs_time = torch.stack(batch_outputs, dim=0)  # shape: (T, batch, 1)
        outputs_mean = outputs_time.mean(dim=0)  # average over timesteps -> (batch, 1)
        preds = (outputs_mean > 0.5).int().cpu().numpy().flatten()
        snn_preds.extend(preds)
        snn_labels.extend(labels.cpu().numpy())
